...
class ElasticInfrastructureGUI:
    ...
    def fetch_infrastructure_data(self):
        ...
        try:
            ...
            try:
                ...
                if isinstance(transforms_data, list):
                    for transform_info in transforms_data:
                        transform_name = transform_info.get('id', transform_info.get('transform_id', 'unknown'))
                        ...
                        if 'source' in transform_info:
                            if 'index' in transform_info['source']:
                                source_index = transform_info['source']['index']
                                if isinstance(source_index, list):
                                    source_index = ', '.join(source_index)

                        if 'dest' in transform_info:
                            if 'index' in transform_info['dest']:
                                dest_index = transform_info['dest']['index']

                        # Extract pivot configuration
                        if 'pivot' in transform_info:
                            if 'aggregations' in transform_info['pivot']:
                                aggregation_config = transform_info['pivot']['aggregations']
                            if 'group_by' in transform_info['pivot']:
                                group_by_config = transform_info['pivot']['group_by']
                    ...
                    # Handle dict format
                    for transform_name, transform_info in transforms_data.items():
                        source_index = None
                        dest_index = None
                        aggregation_config = {}
                        group_by_config = {}
                        ...
                        self.infrastructure_data['transforms'][transform_name] = {
                            ...
                            'stats': transform_info.get('stats', {}),
                            'settings': transform_info.get('settings', {}),
                            'source_indices': set(),  # Will be populated during relationship building
                            'used_by': set()  # Will be populated during relationship building
                        }
            except Exception as e:
            ...
            self.build_transform_relationships()
    ...
    def build_transform_relationships(self):
        """Build relationships between transforms and indices."""
        # Build relationships between transforms and their source/destination indices
        for transform_name, transform_info in self.infrastructure_data['transforms'].items():
            # Handle source indices
            if transform_info['source_index']:
                source_indices = transform_info['source_index']
                if isinstance(source_indices, str):
                    source_indices = [s.strip() for s in source_indices.split(',')]

                for source_index in source_indices:
                    transform_info['source_indices'].add(source_index)

            # Handle destination index relationships
            if transform_info['dest_index']:
                # Add relationship from transform to destination index
                if transform_info['dest_index'] in self.infrastructure_data['indices']:
                    # Mark that this index is created/used by a transform
                    self.infrastructure_data['indices'][transform_info['dest_index']]['created_by_transform'] = transform_name
    ...
    def update_selection_list(self, *args):
        ...
        if analysis_type == "index":
            ...
            items = sorted(self.infrastructure_data['transforms'].keys())
    ...
    def generate_analysis_summary(self, selected_items, analysis_type, visualization_level):
        ...
        for item in selected_items:
            ...
            if analysis_type == 'pipeline':
                ...
                enabled = transform_info.get('enabled', False)
                summary_lines.append(f"• Status: {'✅ Enabled' if enabled else '❌ Disabled'}")

                if transform_info.get('source_index'):
                    summary_lines.append(f"• Source Index: {transform_info['source_index']}")

                if transform_info.get('dest_index'):
                    summary_lines.append(f"• Destination Index: {transform_info['dest_index']}")

                source_count = len(transform_info.get('source_indices', set()))
                summary_lines.append(f"• Source Indices Count: {source_count}")

                # Show transform statistics
                stats = transform_info.get('stats', {})
                if stats.get('documents_processed'):
                    summary_lines.append(f"• Documents Processed: {stats['documents_processed']:,}")

                if stats.get('processing_time_in_ms'):
                    processing_time = stats['processing_time_in_ms'] / 1000  # Convert to seconds
                    summary_lines.append(f"• Processing Time: {processing_time:.2f} seconds")

                if stats.get('documents_indexed'):
                    summary_lines.append(f"• Documents Indexed: {stats['documents_indexed']:,}")

                # Show aggregation configuration summary
                agg_config = transform_info.get('aggregation_config', {})
                if agg_config:
                    agg_count = len(agg_config)
                    summary_lines.append(f"• Aggregations: {agg_count} configured")
                    if agg_count <= 3:
                        for agg_name in list(agg_config.keys())[:3]:
                            summary_lines.append(f"  - {agg_name}")

                # Show group by configuration summary
                group_by_config = transform_info.get('group_by_config', {})
                if group_by_config:
                    group_by_count = len(group_by_config)
                    summary_lines.append(f"• Group By Fields: {group_by_count} configured")
                    if group_by_count <= 3:
                        for group_name in list(group_by_config.keys())[:3]:
        ...
        level_info = {
            "overview": {
                ...
                "legend_items": [
                    ...
                    ("transform", "#e0f7fa", "Transform (Hexagon)")
                ]
            },
            "pipeline_detail": {
            ...
            <script>
                ...
                function showTooltip(position, node) {{
                    ...
                    if (node.type === 'pipeline') {{
                        ...
                        content += `Source Indices: ${{metadata.source_count || 0}}<br/>`;
                        content += `Status: ${{metadata.enabled ? '✅ Enabled' : '❌ Disabled'}}<br/>`;
                        content += `Documents Processed: ${{metadata.processed_docs ? metadata.processed_docs.toLocaleString() : 0}}<br/>`;
                        if (metadata.dest_index) {{
                            content += `Destination Index: ${{metadata.dest_index}}<br/>`;
                        }}
                    }}
                ...
                function updateSidebar(node) {{
                    ...
                    if (node.type === 'pipeline') {{
                        ...
                        content += `
                            <div class="info-section">
                                <h4>📊 Transform Statistics</h4>
                                <div class="stat-item">Source Indices: <strong>${{metadata.source_count || 0}}</strong></div>
                                <div class="stat-item">Status: <strong>${{metadata.enabled ? '✅ Enabled' : '❌ Disabled'}}</strong></div>
                                <div class="stat-item">Documents Processed: <strong>${{metadata.processed_docs ? metadata.processed_docs.toLocaleString() : 0}}</strong></div>
                        `;

                        if (metadata.processing_time) {{
                            content += `<div class="stat-item">Processing Time: <strong>${{metadata.processing_time}} ms</strong></div>`;
                        }}

                        content += `</div>`;

                        if (detailedInfo.source_index) {{
                            content += `
                                <div class="info-section">
                                    <h4>📥 Source Index</h4>
                                    <p>${{detailedInfo.source_index}}</p>
                                </div>
                            `;
                        }}

                        if (detailedInfo.dest_index) {{
                            content += `
                                <div class="info-section">
                                    <h4>📤 Destination Index</h4>
                                    <p>${{detailedInfo.dest_index}}</p>
                                </div>
                            `;
                        }}

                        if (detailedInfo.aggregation_config && Object.keys(detailedInfo.aggregation_config).length > 0) {{
                            content += `
                                <div class="info-section">
                                    <h4>📊 Aggregations</h4>
                                    <ul>${{Object.keys(detailedInfo.aggregation_config).map(agg => `<li>${{agg}}</li>`).join('')}}</ul>
                                </div>
                            `;
                        }}

                        if (detailedInfo.group_by_config && Object.keys(detailedInfo.group_by_config).length > 0) {{
                            content += `
                                <div class="info-section">
                                    <h4>🔗 Group By Fields</h4>
                                    <ul>${{Object.keys(detailedInfo.group_by_config).map(field => `<li>${{field}}</li>`).join('')}}</ul>
                                </div>
                            `;
                        }}
                        ...
                        if (detailedInfo.source_indices && detailedInfo.source_indices.length > 0) {{
                            content += `
                                <div class="info-section">
                                    <h4>📋 All Source Indices</h4>
                                    <ul>${{detailedInfo.source_indices.map(idx => `<li>${{idx}}</li>`).join('')}}</ul>
                                </div>
                            `;
                        }}
                    }} else if (node.type === 'transform') {{
                        // Show comprehensive transform configuration
                        content += `
                            <div class="detail-section">
                                <h3>⚙️ Transform Configuration</h3>
                        `;
                        ...
                        if (detailedInfo.source_index) {{
                            content += `
                                <div class="config-item">
                                    <h4>Source Index</h4>
                                    <code>${{detailedInfo.source_index}}</code>
                                </div>
                            `;
                        }}
                        ...
                        if (detailedInfo.dest_index) {{
                            content += `
                                <div class="config-item">
                                    <h4>Destination Index</h4>
                                    <code>${{detailedInfo.dest_index}}</code>
                                </div>
                            `;
                        }}
                        ...
                        content += `
                                <div class="config-item">
                                    <h4>Status</h4>
                                    <span class="${{detailedInfo.enabled ? 'status-enabled' : 'status-disabled'}}">
                                        ${{detailedInfo.enabled ? '✅ Enabled' : '❌ Disabled'}}
                                    </span>
                                </div>
                            </div>
                        `;
                            ...
                            content += `
                                <div class="detail-section">
                                    ...
                                    <div class="config-json">
                                        <pre>${{JSON.stringify(detailedInfo.aggregation_config, null, 2)}}</pre>
                                    </div>
                                </div>
                            `;
                        }}

                        // Show group by configuration
                        if (detailedInfo.group_by_config && Object.keys(detailedInfo.group_by_config).length > 0) {{
                            content += `
                                <div class="detail-section">
                                    <h3>🔗 Group By Configuration</h3>
                                    <div class="config-json">
                                        <pre>${{JSON.stringify(detailedInfo.group_by_config, null, 2)}}</pre>
                                    </div>
                                </div>
                            `;
                        }}
                        ...
                        if (detailedInfo.stats && Object.keys(detailedInfo.stats).length > 0) {{
                            ...
                            if (stats.documents_processed !== undefined) {{
                                content += `
                                    <div class="stat-card">
                                        ...
                                        <span class="stat-value">${{stats.documents_processed.toLocaleString()}}</span>
                                    </div>
                                `;
                            }}
                            ...
                            if (stats.documents_indexed !== undefined) {{
                                content += `
                                    <div class="stat-card">
                                        <h5>Documents Indexed</h5>
                                        <span class="stat-value">${{stats.documents_indexed.toLocaleString()}}</span>
                                    </div>
                                `;
                            }}

                            if (stats.processing_time_in_ms !== undefined) {{
                                content += `
                                    <div class="stat-card">
                                        <h5>Processing Time</h5>
                                        <span class="stat-value">${{(stats.processing_time_in_ms / 1000).toFixed(2)}}s</span>
                                    </div>
                                `;
                            }}
                            ...
                            if (stats.pages_processed !== undefined) {{
                                content += `
                                    <div class="stat-card">
                                        <h5>Pages Processed</h5>
                                        <span class="stat-value">${{stats.pages_processed.toLocaleString()}}</span>
                                    </div>
                                `;
                            }}
                        ...
                        if (detailedInfo.settings && Object.keys(detailedInfo.settings).length > 0) {{
                            content += `
                                <div class="detail-section">
                                    <h3>🔧 Transform Settings</h3>
                                    <div class="config-json">
                                        <pre>${{JSON.stringify(detailedInfo.settings, null, 2)}}</pre>
                                    </div>
                                </div>
                            `;
                        }}
        ...
        html_content += """
                    ...
                    <div class="legend-section">
                        ...
                        <div class="relationship-types">
                            ...
                            <span class="relationship-type">sources from → Transform Source</span>
                            <span class="relationship-type">creates → Transform Destination</span>
                        </div>
                    </div>
                </div>"""
